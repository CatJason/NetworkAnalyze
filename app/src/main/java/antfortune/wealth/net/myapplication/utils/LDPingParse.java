...
public class LDPingParse {
  public static StringBuilder getFormattingStr(String host, String log) {
    StringBuilder logRes = new StringBuilder();
    logRes.append("Ping: ").append(host).append("\n");
    if (log.contains("timeout")) {
      logRes.append("ping: cannot resolve ").append(host).append(": Timeout");
    } else if (log.contains("unknown")) {
      logRes.append("ping: cannot resolve ").append(host).append(": Unknown host");
    } else {
  ...
  public static void makePingResponse(String log, StringBuilder logRes) {
    ...
    for (int i = 0; i < len - 1; i++) {
      logRes.append(bytesList.get(i)).append("bytes from ").append(hostIp).append(": icmp_seq=#").append(icmpList.get(i)).append(" ttl=").append(ttlList.get(i)).append(" time=").append(timeList.get(i)).append("ms").append("\n");
    }
    logRes.append(bytesList.get(len - 1)).append("bytes from ").append(hostIp).append(": icmp_seq=#").append(icmpList.get(len - 1)).append(" ttl=").append(ttlList.get(len - 1)).append(" time=").append(timeList.get(len - 1)).append("ms");
  }
